...
def _parse_args():
    ...
    parser.add_argument('--func', type=str, default='QUAD', help='function to optimize (QUAD or ROSENBROCK)')
...
class Quadratic(nn.Module):
    """
    Defines the function sum_{i=1}^n (x_i - 1)^2, which is minimized at x = all ones
    """
    ...
    def forward(self, x):
        intermediate = self.weight - torch.from_numpy(np.ones([self.weight.shape[0]])).float()
        return intermediate.dot(intermediate)
